...
CREATE PROCEDURE mtc_getAllIndexesWithAFragmentationOverThan20Percent(@databaseName varchar(50) = 'ALL')
AS
BEGIN
  SET NOCOUNT ON
...
CREATE PROCEDURE mtc_reorgonizeAndRebuildIndexes(@debugMode bit = 0, @mode varchar(50) = 'DETAILED')
AS
BEGIN
  ...
  -- TODO: add condition for stop execution by time
  -- TODO: add MacDOP option for defragmentation and rebuild
  ...
  DECLARE DefragList CURSOR FOR
    ...
    FROM _Maintenance.dbo.med_info_index_detail
    ORDER BY frag DESC
  FOR READ ONLY
